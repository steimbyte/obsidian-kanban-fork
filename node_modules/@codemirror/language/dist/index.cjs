...
class StringStream {
    ...
    indentUnit, lookAhead, overrideIndent) {
        this.string = string;
    ...
    getIndent(cx) {
        ...
        while (statePos < cx.pos) {
            ...
            if (line.length) {
                ...
                let lookahead = (n) => n + line.number > cx.state.doc.lines ? '' : cx.state.doc.line(n + line.number).text;
...
class Parse {
    ...
    advance() {
        ...
        while (this.parsedPos < end)
            parseNext = this.parseLine(context);
        while (parseNext && this.parsedPos < this.to)
            parseNext = this.parseLine(context);
...
const tokenClassNodeProp = new common.NodeProp();
const lineClassNodeProp = new common.NodeProp();
class LineHighlighter {
    ...
    buildDeco(view$1) {
        if (!this.tree.length)
            return view.Decoration.none;
        let spellcheckIgnoreTokens = [];
        for (let str of view$1.state.facet(ignoreSpellcheckToken)) {
            spellcheckIgnoreTokens.push(...str.split(' '));
        }
        let spellCheckIgnoreSet = new Set(spellcheckIgnoreTokens);
        ...
        for (let { from, to } of view$1.visibleRanges) {
            this.tree.iterate({
                ...
                enter: ({ type, from, to }) => {
                    ...
                    if (lineStyle) {
                        let lineDeco = this.lineCache[lineStyle] || (this.lineCache[lineStyle] = view.Decoration.line({ attributes: { class: lineStyle } }));
                        let newFrom = view$1.lineBlockAt(from).from;
...
const lineHighlighter = state.Prec.lowest(view.ViewPlugin.define(view => new LineHighlighter(view), {
    decorations: v => v.decorations
}));
const ignoreSpellcheckToken = state.Facet.define({});
...
exports.ignoreSpellcheckToken = ignoreSpellcheckToken;
...
exports.lineClassNodeProp = lineClassNodeProp;
...
exports.tokenClassNodeProp = tokenClassNodeProp;
